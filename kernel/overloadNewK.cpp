 ...
 * @FilePath: /dolphin3/kernel/overloadNewK.cpp
 * @Description: 所有的在kernel空间中new的内存都会从这里分配，所以所有class必须继承Kernel::NewK
*/
...
int NewK::_blockBitMap64[0x2000 / 0x40] = {0};
int NewK::_blockBitMap128[0x2000 / 0x80] = {0};
int NewK::_blockBitMap256[0x2000 / 0x100] = {0};

void *NewK::operator new(size_t size)
{
...
void NewK::operator delete(void *p)
{
...
int NewK::allocate_mem(size_t size)
{
    if (size <= 0x40)
    {
...
int NewK::free_mem(uint64_t addr)
{
    // 起始地址是0x201000，结束地址是0x207000
